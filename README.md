...
- [Get device features](DOCUMENTATION.md#devGetFeatures) and check that:
  ...
  * `model` is set to `'1'`
  * `fwMajor` is set to expected firmware major number
  * `fwMinor` is set to expected firmware minor number
  * `fwPatch` is set to expected firmware patch number
...
- [Get device features](DOCUMENTATION.md#devGetFeatures) and:
  * `vendor` is set to `Skycoin Foundation`
