...
- Ensure you know at least two addresses for test wallet, if not, [generate some](DOCUMENTATION.md#devAddressGen). Choose two of them, hereinafter referred to as `ADDRESS2`, `ADDRESS3`
...
```
- From command output take note of first wallet address `ADDRESS4`
...
```
- Use Skycoin REST API to create one transaction (a.k.a `TXN3_JSON`) grabbing all funds from `ADDRESS1` (i.e. first address in `WALLET1` previously recovered in Skywallet device), `ADDRESS2` (i.e. second address in `WALLET1` previously recovered in Skywallet device) and `ADDRESS4` (i.e. first address in `WALLET2`) so as to transfer to the third address of `WALLET1` (i.e. `ADDRESS3`). Change should be sent back to `ADDRESS1`. If Skycoin node was started with default parameters this can be achieved as follows:
```sh
